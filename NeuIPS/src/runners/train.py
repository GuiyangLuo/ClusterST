...
def weights_init(m):
    ...
    if classname.find('Linear') != -1:
        # print('initializing for ', classname)
...
def train(args):
    ...
    if not os.path.exists(model_save_dir):
        ...
        if model_path is not None:
            ...
            print("Begin to training a new model...", flush=True)
    ...
    for i in range(begin_epoch,args.epochs+1):
        weights = math.pow(0.97, np.maximum(2, i) - 2)
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            metrics = engine.train(trainx, trainy[:,0,:,:], weights = weights)

            train_loss.append(metrics[0])
            train_mape.append(metrics[1])
            train_rmse.append(metrics[2])
            if iter % args.print_every == 0 :
                log = 'Iter: {:03d}, Weights: {:.4f},  Train Loss: {:.4f}, Train MAPE: {:.4f}, Train RMSE: {:.4f}'
                print(log.format(iter, weights, train_loss[-1], train_mape[-1], train_rmse[-1]),flush=True)
