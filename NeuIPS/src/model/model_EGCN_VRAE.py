...
class EmbedGCN(nn.Module):
    ...
    def __init__(self, args,  road_sensor_pos, adj_mx_topk_index,  writer =None):
        ...
        self.clusterLoss = ClusterLoss(args.neighbors_for_each_nodes, args.gcn_n_class, adj_mx_topk_index,  device = torch.device(args.device), margin=0.1)
    ...
    def forward(self, input):
        ...
        vrae_loss, latent_mean = self.vrae_net(input_for_vrae)
        ...
        adjacency_mat_topk_ids = (adjacency_mat_topk - adjacency_mat_topk.min()) / adjacency_mat_topk.max()
        adjacency_mat_topk_ids = torch.clamp(adjacency_mat_topk_ids, 0, 1 - 0.001) * self.gcn_n_class
        adjacency_mat_topk_ids = torch.floor(adjacency_mat_topk_ids)

        threeDinput = self.cluster_GCN(adjacency_mat_topk, adjacency_mat_topk_ids,  weightedDinput_topk)
        # print("  threeDinput ", threeDinput.shape)
        ...
        print(" adjacency_mat_topk_ids ",adjacency_mat_topk_ids[0,0,:])
        cluster_loss = self.clusterLoss( adjacency_mat_topk, adjacency_mat_topk_ids)
        # print(" cluster_loss and  vrae_loss", cluster_loss, vrae_loss)
        return output, cluster_loss + vrae_loss
...
class Ordered_GCN(nn.Module):
    ...
    def forward(self, adjacency_mat_topk,adjacency_mat_topk_ids, weightedDinput_topk, ):
        adjacency_mat_topk_ids = torch.unsqueeze(adjacency_mat_topk_ids, -1)
        adjacency_mat_topk_ids_expanded = adjacency_mat_topk_ids.expand(-1, -1, -1, self.seq_in_len)

        out_list = []

        for i, net_block in enumerate(self.linears):
            threeDinput = torch.where(adjacency_mat_topk_ids_expanded == i, weightedDinput_topk, self.constant_zeros)
            class_count = torch.eq(adjacency_mat_topk_ids_expanded, i).to(torch.float32).sum(dim=2, keepdim=False)
            class_count = torch.where(class_count == self.constant_zeros, self.constant_ones, class_count)
            # print("  ------",threeDinput.shape, class_count.shape )
            threeDinput = torch.div(threeDinput.sum(dim=2, keepdim=False), class_count)
...
class ClusterLoss(nn.Module):
    ...
    def __init__(self, neighbors_for_each_nodes, gcn_n_class, adj_mx_topk_index ,  device , margin=0.5):
        ...
        self.adj_mx_topk_index = adj_mx_topk_index  # shape: (batch, sensors, neighbors_for_each_nodes)
        self.constant_zeros = torch.zeros([1]).to(device)
    def forward(self, adjacency_mat_topk, adjacency_mat_topk_ids):
        # print("1111 adjacency_mat_topk ", adjacency_mat_topk[0,0,:])

        totoal_loss = []
        for i in range(self.gcn_n_class):
            masks = torch.eq(adjacency_mat_topk_ids, i)
            class_count = masks.to(torch.float32).sum(dim=-1)
            # print("---------------------")
            # print("3333 class_count ",i, class_count[0, 0])
            values = torch.where(adjacency_mat_topk_ids==i, adjacency_mat_topk,  self.constant_zeros )
            # print("3333 class_count ", i, values[0, 0,:])
            # print(" values ", i, values[0, 0, :])
            values = torch.std(values, dim = -1)
            # class_count = torch.clamp(class_count, 1,  self.gcn_n_class)
            loss_i = torch.mul(values, class_count)
            totoal_loss.append(loss_i)

        return torch.cat(totoal_loss).mean()