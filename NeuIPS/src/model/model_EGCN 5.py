...
class clustering_attention_dynamic_learning2(nn.Module):
    ...
    def vis_cluster_result(self,attention,  prob, distance, print_every=300, ):
        ...
        if self.writer and self.global_terater % print_every == 0:
            ...
            import matplotlib.pyplot as plt
            plt.switch_backend('agg')
            fig,  (ax0, ax1,ax2)= plt.subplots(figsize=(10,10),nrows=3)
    ...
    def forward(self, fushed_features, input_data):
        ...
        attention_mask = attention.softmax(dim = -1)
        attention_mask_transpose = attention_mask.transpose(-1,-2)
        ...
        wh_expand_top_k = wh_expand_top_k.detach() # wh_expand_top_k.detach()  or  input_data_expand_top_k
        if self.sim_metric == 'euc':
            dist_mat = self.euc_sim(wh_expand_top_k, wh_expand_top_k)
        elif self.sim_metric == 'cos':
            dist_mat = self.cos_sim(wh_expand_top_k, wh_expand_top_k)
        ...
        # prob_dist = torch.mul(dist_mat.unsqueeze(-1), prob_i_j)
        # prob_dist_sum = prob_dist.permute(0, 1, 4, 2, 3).triu(diagonal=1)
        # prob_i_j = prob_dist_sum.sum(-1).sum(-1)
        # prob_i_j = prob_i_j.sum(-1)
        # cluster_loss = prob_i_j.mean()

        probability = torch.matmul(attention_mask, attention_mask_transpose)
        self.vis_cluster_result(attention_mask, probability, dist_mat)

        probability = probability.clamp(min=1e-4, max=1 - 1e-4)

        probability_int = probability.ge(0.04).float()
        distance_mat = 1 - self.cos_sim(attention_mask,attention_mask)

        cluster_diff_loss = torch.mul(distance_mat, probability_int) + torch.mul(torch.clamp(0.5 - distance_mat, min =0 ), (1 - probability_int))

        cluster_diff_loss = (cluster_diff_loss.sum(-1).sum(-1) - torch.einsum('...ii', cluster_diff_loss)).mean()

        return output_data, [ cluster_loss, cluster_diff_loss, wh.mean(), attention.mean(), dist_mat.mean()]

class clustering_attention_each_class(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, margin = 1, alpha = 0.5,  dim_out = 6, seq_out = 12, writer = None ):
        super(clustering_attention_each_class, self).__init__()
        device = torch.device(args.device)
        self.writer = writer
        self.global_terater = 0
        self.dim_out = dim_out
        ...
        self.W = nn.Parameter(torch.empty(size=(args.seq_length_x, seq_out)))
        # nn.init.xavier_uniform_(self.W.data, gain=1.414)
        nn.init.uniform_(self.W.data, a=0, b=1)
        self.a = nn.Parameter(torch.empty(size=(2 * seq_out, self.gcn_n_class)))
        # nn.init.xavier_uniform_(self.a.data, gain=1.414)
        nn.init.uniform_(self.a.data, a=0, b=1)
        self.leakyrelu = nn.LeakyReLU(alpha)
    ...
    def forward(self, fushed_features, input_data):
        ...
        wh_expand = wh.unsqueeze(1).expand(-1, self.number_of_sensors, -1, -1)
        wh_expand_top_k = torch.gather(wh_expand, 2, self.adj_mx_topk_index_expanded_seq)
        return attention_mask, wh_expand_top_k
...
class clustering_attention_dynamic_learning3(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, margin = 1, alpha = 0.5,  dim_out = 6, seq_out = 12, writer = None ):
        ...
        self.adj_mx_topk_index_expanded_seq = adj_mx_topk_index.unsqueeze(-1).expand(-1, -1, -1,
                                                                                        args.seq_length_x)
        self.multi_head_attention =  nn.ModuleList([clustering_attention_each_class(args, fused_dim, adj_mx_topk_index) for i in range(args.gcn_n_class)])
    ...
    def forward(self, fushed_features, input_data):
        ...
        out_list = []
        for i in range(self.gcn_n_class):
            attention,_ = self.multi_head_attention[i](fushed_features, input_data)
            out_list.append(attention.unsqueeze(-1))
        attention_cat = torch.cat(out_list, dim=-1)
        attention = attention_cat.max(dim = -1)[0]

        # attention_mask = attention / torch.clamp(attention.sum(dim = -1, keepdim=True),min=1e-5)
        attention_mask = attention.softmax(-1)
        ############------------------Loss for clustering---------------################
        # based on attention_mask, whose shape is (batch, sensors, neighbors, class )
        attention_mask_transpose = attention_mask.transpose(-1,-2)
        input_data_expand = input_data.expand(-1, self.number_of_sensors, -1, -1)
        ...
        wh_expand_top_k = input_data_expand_top_k.detach() # wh_expand_top_k.detach()  or  input_data_expand_top_k
        if self.sim_metric == 'euc':
            dist_mat = self.euc_sim(wh_expand_top_k, wh_expand_top_k)
        elif self.sim_metric == 'cos':
            dist_mat = self.cos_sim(wh_expand_top_k, wh_expand_top_k)
        ...
        attention_mask_i = attention_mask.unsqueeze(-2).expand(-1,-1,-1,self.neighbors_for_each_nodes,-1)
        attention_mask_j = attention_mask.unsqueeze(-3).expand(-1, -1, self.neighbors_for_each_nodes, -1, -1)
        prob_i_j = torch.mul(attention_mask_i, attention_mask_j)
        prob_i_j = prob_i_j.softmax(-1)
        # prob_i_j = torch.where(prob_i_j<0.09, torch.zeros_like(prob_i_j), prob_i_j)
        # print("111--shape--", attention_mask[0,0,:,:], prob_i_j[0,0,:2,:2,:])
        ...
        return output_data, [ cluster_loss, cluster_diff_loss, prob_i_j.mean(), attention.mean(), dist_mat.mean()]

class clustering_attention_dynamic_learning4(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, cluster_feature_dim = 18, margin = 1, alpha = 0.5,  dim_out = 6, seq_out = 12, writer = None ):
        super(clustering_attention_dynamic_learning4, self).__init__()
        device = torch.device(args.device)
        self.sim_metric = args.simi_metric
        self.writer = writer
        self.global_terater = 0
        ...
        self.centroids = nn.Parameter( (torch.randn([ args.gcn_n_class, cluster_feature_dim],dtype=torch.float)))
        nn.init.uniform_(self.centroids.data, a=0, b=1)

        self.wh_func = nn.Sequential(nn.Linear(args.seq_length_x*args.gcn_n_class, cluster_feature_dim, bias = False), nn.ReLU())
    ...
    def forward(self, fushed_features, input_data):
        ...
        wh_list = []
        for i in range(self.gcn_n_class):
            attention, wh_expand_top_k = self.multi_head_attention[i](fushed_features, input_data)
            out_list.append(attention.unsqueeze(-1))
            wh_list.append(wh_expand_top_k)
        attention_cat = torch.cat(out_list, dim=-1)
        attention = attention_cat.max(dim = -1)[0]
        ...
        ############------------------Loss for clustering---------------################
        # based on attention_mask, whose shape is (batch, sensors, neighbors, class )
        wh_cat = torch.cat(wh_list, dim=-1)
        wh_cat = wh_cat.detach()
        # print(" shape ", wh_cat.shape, attention.shape)
        wh_cat_feature = self.wh_func(wh_cat)
        new_centroids = self.centroids.unsqueeze(0).unsqueeze(0).expand(self.batch_size, self.number_of_sensors,-1,-1)
        # print(" shape -", wh_cat_feature.shape,new_centroids.shape )
        if self.sim_metric == 'euc':
            dist_mat = self.euc_sim(wh_cat_feature, new_centroids)
        elif self.sim_metric == 'cos':
            dist_mat = self.cos_sim(wh_cat_feature, new_centroids)
            dist_mat = 1 - dist_mat
        dist_mat = dist_mat.softmax(-1)
        # print("000000--shape--", attention_mask.min(), attention_mask.max(),attention_mask.mean())
        cluster_loss = torch.pow(dist_mat - attention, 2).sum(-1).sum(-1)

        cluster_loss = cluster_loss.mean()


        return output_data, [ cluster_loss, wh_cat_feature.mean(), new_centroids.mean(), attention.mean(), dist_mat.mean()]

class clustering_attention_dynamic_learning5(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, cluster_feature_dim = 12, margin = 1, alpha = 0.5,  dim_out = 6, seq_out = 12, writer = None ):
        super(clustering_attention_dynamic_learning5, self).__init__()
        device = torch.device(args.device)
        self.sim_metric = args.simi_metric
        self.writer = writer
        self.global_terater = 0
        ...
        self.centroids = nn.Parameter( (torch.randn([ args.gcn_n_class, cluster_feature_dim],dtype=torch.float)))
        nn.init.normal(self.centroids.data, std=0.001)

        self.wh_func = nn.Sequential(nn.Linear(args.seq_length_x, cluster_feature_dim, bias = False))
        self.a_func = nn.Sequential(nn.Linear(cluster_feature_dim*2, 1, bias=False), nn.LeakyReLU())
    ...
    def forward(self, fushed_features, input_data):
        ...
        input_feature = self.wh_func(input_data_expand_top_k)

        input_feature_expanded = input_feature.unsqueeze(-2).expand(-1,-1,-1,self.gcn_n_class,-1)
        new_centroids_expanded = self.centroids.unsqueeze(0).unsqueeze(0).unsqueeze(0).expand(self.batch_size, self.number_of_sensors,self.neighbors_for_each_nodes, -1,-1)
        new_centroids_expanded = new_centroids_expanded.detach()
        cat_feature = torch.cat([input_feature_expanded, new_centroids_expanded], dim = -1)

        attention = self.a_func(cat_feature)
        attention = attention.squeeze()
        attention = attention.softmax(-1)
        self.vis_cluster_result(attention, attention, attention)
        dist_mat_transpose = attention.transpose(-1,-2)
        output_data = torch.matmul(dist_mat_transpose, input_feature)

        ################KL loss
        # centroids_x = self.centroids.unsqueeze(0).expand(self.gcn_n_class, -1, -1)
        # centroids_y = self.centroids.unsqueeze(1).expand(-1,  self.gcn_n_class,  -1)
        # clustering_loss_ori = nn.functional.kl_div(centroids_x.softmax(dim=-1).log(), centroids_y.softmax(dim=-1), reduction='none')
        # clustering_loss = torch.triu(clustering_loss_ori, diagonal=1)
        # clustering_loss = clustering_loss.sum()


        clustering_loss_ori = self.euc_sim(self.centroids, self.centroids)
        clustering_loss = torch.triu(clustering_loss_ori, diagonal=1)
        target = torch.zeros_like(clustering_loss).fill_(1).triu_(diagonal=1)
        clustering_loss = (clustering_loss- target).pow_(2).sum()


        return output_data, [clustering_loss, input_data_expand.mean(),input_feature.mean(),self.centroids.mean(), output_data.mean()]

class ExternalAttention(nn.Module):

    def __init__(self, d_model_in,  S=24):
        super().__init__()
        self.mk=nn.Linear(d_model_in,S,bias=False)
        self.mv=nn.Linear(S,d_model_in,bias=False)
    ...
    def forward(self, queries):
        ...
        attn=attn / torch.clamp(torch.sum(attn,dim=-1,keepdim=True),min = 1e-4)
        out=self.mv(attn) #bs,n,d_model
        return out
class clustering_attention_dynamic_learning6(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, cluster_feature_dim = 12, margin = 1, alpha = 0.5,  dim_out = 6, seq_out = 12, writer = None ):
        super(clustering_attention_dynamic_learning6, self).__init__()
        device = torch.device(args.device)
        self.sim_metric = args.simi_metric
        self.writer = writer
        self.global_terater = 0
        ...
        self.adj_mx_topk_index_expanded_seq = adj_mx_topk_index.unsqueeze(-1).expand(-1, -1, -1,
                                                                                        args.seq_length_x)
        self.multi_head_attention =  nn.ModuleList([clustering_attention_each_class(args, fused_dim, adj_mx_topk_index) for i in range(args.gcn_n_class)])
        self.e_attention = ExternalAttention(args.seq_length_x)
        # self.weight_init()
    ...
    def forward(self, fushed_features, input_data):
        ...
        input_data_expand = input_data.expand(-1, self.number_of_sensors, -1, -1)
        input_data_expand_top_k = torch.gather(input_data_expand, 2, self.adj_mx_topk_index_expanded_seq)
        output_data = self.e_attention(input_data_expand_top_k)

        return output_data, [input_data.max(), input_data.mean(),output_data.max(),output_data.min(), output_data.mean()]


class ExternalAttention_inout(nn.Module):

    def __init__(self, d_model_in,d_model_out,  S=12*7):
        super().__init__()
        self.mk=nn.Linear(d_model_in,S,bias=False)
        self.mv=nn.Linear(S,d_model_out,bias=False)
        self.softmax=nn.Softmax(dim=-2)
        # self.init_weights()
...
class clustering_attention_dynamic_learning6(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, cluster_feature_dim = 12, margin = 1, alpha = 0.5,  dim_out = 6, seq_out = 12, writer = None ):
        ...
        self.adj_mx_topk_index_expanded_seq = adj_mx_topk_index.unsqueeze(-1).expand(-1, -1, -1,
                                                                                        args.seq_length_x)
        self.multi_head_attention =  nn.ModuleList([clustering_attention_each_class(args, fused_dim, adj_mx_topk_index) for i in range(args.gcn_n_class)])
        self.e_attention = ExternalAttention_inout(args.seq_length_x*self.neighbors_for_each_nodes,args.seq_length_x*self.gcn_n_class, )
        # self.weight_init()
    ...
    def forward(self, fushed_features, input_data):
        ...
        input_data_expand = input_data.expand(-1, self.number_of_sensors, -1, -1)
        input_data_expand_top_k = torch.gather(input_data_expand, 2, self.adj_mx_topk_index_expanded_seq)
        input_data_expand_top_k = input_data_expand_top_k.view(self.batch_size, self.number_of_sensors, -1)
        output_data = self.e_attention(input_data_expand_top_k)
        output_data = output_data.view(self.batch_size, self.number_of_sensors,  self.gcn_n_class, -1,)
        return output_data, [input_data.max(), input_data.mean(),output_data.max(),output_data.min(), output_data.mean()]

class clustering_attention_dynamic_learning7(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, cluster_feature_dim = 12,  dim_middle = 12*7, seq_out = 12, writer = None ):
        super(clustering_attention_dynamic_learning7, self).__init__()
        device = torch.device(args.device)
        self.sim_metric = args.simi_metric
        self.writer = writer
        self.global_terater = 0
        ...
        self.mk = nn.Linear(args.seq_length_x*args.neighbors_for_each_nodes, dim_middle, bias=False)
        self.mv = nn.Linear(dim_middle, args.seq_length_x*args.gcn_n_class, bias=False)
        self.softmax = nn.Softmax(dim=-2)
    ...
    def forward(self, fushed_features, input_data):
        ...
        attn = self.mk(input_data_expand_top_k)  # bs,n,S
        attn = self.softmax(attn)  # bs,n,S
        # attn = attn / torch.clamp(torch.sum(attn, dim=-1, keepdim=True), min=1e-2)
        # attn = attn.clamp(max=2)
        output_data_attn = self.mv(attn)  # bs,n,d_model
        output_data = output_data_attn.view(self.batch_size, self.number_of_sensors,  self.gcn_n_class, -1,)
        return output_data, [input_data.max(), input_data.mean(),output_data.max(),output_data.min(), output_data.mean()]

class clustering_attention_dynamic_learning8(nn.Module):
    def __init__(self, args, fused_dim, adj_mx_topk_index, cluster_feature_dim = 12,  dim_middle = 12*7, seq_out = 12, writer = None ):
        super(clustering_attention_dynamic_learning8, self).__init__()
        device = torch.device(args.device)
        self.sim_metric = args.simi_metric
        self.writer = writer
        self.global_terater = 0
        ...
        # self.clustering_learning = nn.Linear(args.seq_length_x,
        #                                      args.gcn_n_class, bias=False)
        self.clustering_learning = nn.Linear(args.seq_length_x * args.neighbors_for_each_nodes,  args.neighbors_for_each_nodes * args.gcn_n_class, bias=False)
    ...
    def vis_cluster_result(self, weight1,  weight2,  weight3, number = 5,  print_every=300,  ):

        self.global_terater = self.global_terater + 1
        if self.writer and self.global_terater % print_every == 0:
            weight2_data = weight2.view(number, -1, weight2.shape[1])
            weight1 = weight1.detach().cpu().numpy()
            weight2_data = weight2_data.detach().cpu().numpy()
            weight2 = weight2.detach().cpu().numpy()
            weight3 = weight3.detach().cpu().numpy()
            import matplotlib.pyplot as plt
            plt.switch_backend('agg')
            fig,  ax = plt.subplots(figsize=(10,10),nrows=number + 3)
            weight1_data = weight1[0,0,:,:]
            weight3_data = weight3[0, 0, :, :]

            sns.heatmap(weight1_data, linewidths=0.05, ax= ax[0], fmt='.2f', annot=True, cmap="RdBu_r")
            sns.heatmap(weight3_data,  linewidths=0.05, ax=ax[1], fmt='.2f', annot=True, cmap="RdBu_r")
            sns.heatmap(weight2, linewidths=0.05, ax=ax[2], fmt='.2f', annot=False, cmap="RdBu_r")
            for i in range(number):
                sns.heatmap(weight2_data[i,...], linewidths=0.05, ax=ax[3+i], fmt='.2f', annot=False, cmap="RdBu_r")
    ...
    def forward(self, fushed_features, input_data):
        ...
        input_data_expand = input_data.expand(-1, self.number_of_sensors, -1, -1)
        input_data_expand_top_k = torch.gather(input_data_expand, 2, self.adj_mx_topk_index_expanded_seq)
        input_data_expand_top_k_fold = input_data_expand_top_k.view(self.batch_size, self.number_of_sensors, -1)
        attn = self.clustering_learning(input_data_expand_top_k_fold)
        attn = self.act_func(attn)
        attn = attn.view(self.batch_size, self.number_of_sensors,  self.gcn_n_class, -1,)
        attn_sfm = attn.softmax(-2)
        output_data = torch.matmul(attn_sfm,input_data_expand_top_k )

        self.vis_cluster_result(attn , self.clustering_learning.weight, attn_sfm)
