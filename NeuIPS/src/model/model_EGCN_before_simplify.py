...
class EmbedGCN(nn.Module):
    ...
    def __init__(self, args,  road_sensor_pos, adj_mx_topk_index,  writer =None):
        ...
        ### for VREA
        self.vrae_net = VRAE_Model(args.seq_length_y, args.in_dims, args.vrae_hidden_size, args.vrae_hidden_layer_depth, args.vrae_latent_length, args.batch_size*args.number_of_sensors, block = args.vrae_rnn_model, dropout_rate = args.vrae_dropout_rate, device = torch.device(args.device) )
        self.contrastiveLoss = ContrastiveLossNew2(args, args.neighbors_for_each_nodes, adj_mx_topk_index, self.feature_size, margin=0.1)
    ...
    def forward(self, input):
        ...
        # output = output.view(batch_size, number_sensors, -1, seq_in)
        # output = output.permute(0, 3, 1, 2)
        contrastiveLoss_out = self.contrastiveLoss( fushed_features, nodes_pairs_classes_details)
        return output, contrastiveLoss_out
class ContrastiveLossNew2(nn.Module):

    def __init__(self, args, neighbors_for_each_nodes, adj_mx_topk_index , feature_size, margin=0.5):
        super(ContrastiveLossNew2, self).__init__()
    ...
    def forward(self, fusedFeatures, nodes_pairs_classes_details):
        ...
        loss_contrastive = torch.mean(positive + negtive)
        return loss_contrastive


class ContrastiveLossNew(nn.Module):

    def __init__(self, args, neighbors_for_each_nodes, adj_mx_topk_index , margin=0.5):
    ...
    def forward(self, adjacency_mat, nodes_pairs_classes_details):
       """
       :param adjacency_mat: (batch, sensor, sensor)
       :param nodes_pairs_classes_details:  (batch, sensor, sensor, class)
       :return:
       """
       node_i, node_j = 1,2
       a_i, a_j = self.adj_mx_topk_index[0,2,node_i], self.adj_mx_topk_index[0,2,node_j]

       adjacency_mat_topk = torch.gather(adjacency_mat, 2, self.adj_mx_topk_index)
       adjacency_mat_topk_expanded = adjacency_mat_topk.unsqueeze(1).expand(-1, self.number_of_sensors,-1,-1)
       adj_mx_topk_index_neighbors = self.adj_mx_topk_index.unsqueeze(-1).expand(-1, -1, -1, self.neighbors_for_each_nodes, )
       adjacency_mat_topk_expanded = torch.gather(adjacency_mat_topk_expanded, 2,
                                                  adj_mx_topk_index_neighbors.to(torch.long))

       # print(" Validation of adjacency_mat_topk_expanded",adjacency_mat_topk_expanded[0,2, node_i, node_j],  )


       adj_mx_topk_index_class = self.adj_mx_topk_index.unsqueeze(-1).expand(-1, -1,-1, self.gcn_n_class,)
       nodes_pairs_classes_details = torch.gather(nodes_pairs_classes_details, 2,adj_mx_topk_index_class)

       nodes_pairs_classes_details_1 = nodes_pairs_classes_details.unsqueeze(2).expand(-1, -1, self.neighbors_for_each_nodes,-1,-1)
       nodes_pairs_classes_details_2 = nodes_pairs_classes_details.unsqueeze(3).expand(-1, -1, -1, self.neighbors_for_each_nodes, -1)
       # print("nodes_pairs_classes_details_1",nodes_pairs_classes_details_1.shape, nodes_pairs_classes_details_2.shape, adjacency_mat_topk_expanded.shape)
       nodes_pairs_classes_equals  = torch.mul(nodes_pairs_classes_details_1, nodes_pairs_classes_details_2)

       nodes_pairs_classes_equals = nodes_pairs_classes_equals.sum(-1)

       loss_contrastive = torch.mean((nodes_pairs_classes_equals) * torch.pow(adjacency_mat_topk_expanded,2) +  # calmp夹断用法
                                      (1- nodes_pairs_classes_equals) * torch.pow(torch.clamp(self.margin - adjacency_mat_topk_expanded, min=0.0), 2))
