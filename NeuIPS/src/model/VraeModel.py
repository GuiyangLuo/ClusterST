...
class Decoder(nn.Module):
    ...
    """
    def __init__(self, sequence_length, batch_size, hidden_size, hidden_layer_depth, latent_length, output_size, device = torch.device('cuda:1') , block='LSTM',   ):
...
class VRAE( nn.Module):
    ...
    def __init__(self, sequence_length, number_of_features, hidden_size=90, hidden_layer_depth=2, latent_length=20,
                 batch_size=32, block='LSTM', dropout_rate=0., loss = 'MSELoss', device= torch.device('cuda:1')):
    ...
    def forward(self, x):
        ###   x: input to the encoder, of shape (sequence_length, batch_size, number_of_features)
        ...
        # print("  kl_loss + recon_loss ", kl_loss,  kl_loss.shape,recon_loss,  recon_loss.shape)
        return total_loss, torch.cat([latent_mean, latent_logvar],dim = -1)