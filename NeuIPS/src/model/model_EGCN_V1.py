...
class EmbedGCN(nn.Module):
    ...
    def forward(self, input):
        ...
        input = input.permute(0,  1,2, 3).expand(-1,  self.args.number_of_sensors, -1,-1)  # input shape (batch_size, #edges, #edges,in_seq)
        ...
        contrastiveLoss_out = self.contrastiveLoss(clustered_index, adjacency_mat)
...
class Nodes_Clustering_Euclidean(nn.Module):
    ...
    def forward(self, adjacency_mat):
        adjacency_mat = adjacency_mat*self.gcn_n_class
        return torch.ceil(adjacency_mat)
...
class Ordered_GCN(nn.Module):
    ...
    def forward(self,class_for_sensor_index, input):
        # print("----------",class_for_sensor_index.shape, input.shape)
        key_query_class_index = class_for_sensor_index.to(torch.float32)
        # print("-1111111--------key_query_class_index",key_query_class_index[0,0,:5])
        class_for_sensor_index = torch.unsqueeze(class_for_sensor_index, -1)
        key_query_class_index_expand = class_for_sensor_index.expand(-1, -1, -1, self.seq_out_len)
        # print("-22222--------key_query_class_index_expand", key_query_class_index_expand[0, 0, :5,:])
        if True:
            input_expanded = torch.gather(input, 2, key_query_class_index_expand)
        # print("-33333--------input", input[0, 0, :2, :])
        # print("-444444--------input", input_expanded[0, 0, :2, :])
        out_list = []

        for i, net_block in enumerate(self.linears):

            threeDinput = torch.where(key_query_class_index_expand == i, input,  self.constant_zeros)
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, clustered_ids, adjacency_mat,):
        """
        :param self:
        :param fused_features: shape: (batch, sensors,  dim_feature)
        :param clustered_index: shape: (batch, sensors,  sensor)
        :param adjacency_mat: shape: (batch, sensors, sensors)
        :return:
        """
        # print(" clustered Ids ", clustered_ids[0,0,:] )
        clustered_ids_filtered = torch.gather(clustered_ids,2,self.adj_mx_topk_index)
        adjacency_dist_filtered = torch.abs(torch.gather(adjacency_mat, 2, self.adj_mx_topk_index))
